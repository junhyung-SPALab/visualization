...
def parse_config():
    ...
    parser.add_argument('--category', type=str, default=None, help='train, val')
    parser.add_argument('--view', type=str, default=None, help='3D view, BEV')
    parser.add_argument('--num_scene', type=int, default=None, help='Num. of total scene')
    ...
    parser.add_argument('--is_augmentation', type=bool, default=False, help='Select whether to apply augmented visualization')
...
def gt_v2(cfgs, scene_info, frame_info, tag = 'ori'):
    #image = cv2.imread(image_file, cv2.COLOR_BGR2RGB) # X    
    #points = np.fromfile(str(lidar_path), dtype=np.float32, count=-1).reshape([-1, 5])[:,:3]
    lidar_path          = cfgs['lidar_path']
    class_label_path    = cfgs['class_label_path']
    bbox_label_path     = cfgs['corner_path'] 
    ...
    fig = mlab.figure(bgcolor=(0, 0, 0), size=(800, 800)) #default = size=(1280, 720)
    view_type = cfgs['view_type']
    ...
    mlab.view(azimuth=170,elevation=0,distance=160, roll=None,focalpoint=(5,0,10),reset_roll=True)
    
    
    save_path = cfgs['save_path']
    save_dir_name = f'scene_{scene_info}'
    if not os.path.isdir(save_path + save_dir_name):
       os.mkdir(save_path + save_dir_name)
    
    
    if tag == 'ori':
        mlab.savefig(filename=f'{save_path}{save_dir_name}/{frame_info}_ori.jpg', size=(4000,3500), magnification=5)
    elif tag == 'aug':
        mlab.savefig(filename=f'{save_path}{save_dir_name}/{frame_info}_aug.jpg',size=(4000,3500), magnification=5)
    else:
        pass
    #mlab.show()
    mlab.close(fig)
...
def plot_nuscenes_BEV_v2(fig,corners_3d,class_label):
        ...
        def draw(p1, p2, class_label):
            ...
            mlab.plot3d([p1[0], p2[0]], [p1[1], p2[1]], [0, 0], color=nuscene_class_to_color[class_label], tube_radius=None, line_width=7, figure=fig)
...
if __name__ == '__main__':
    ...
    if cfgs['dataset_type'] == 'nuscenes':
        if cfgs['dataset_category'] == 'val':
            base_path = '/nuscenes/val_set/'
        elif cfgs['dataset_category'] == 'train':
            base_path = '/nuscenes/train_set/'
            cfgs['is_augmentation'] = True
        else:
            raise Exception('Dataset의 Category를 정확히 지정해주세요.')

        sample_path = base_path + 'samples/'
        bbox_label_path = base_path + 'corners/'
        class_label_path = base_path + 'class_label/'
        cfgs['save_path'] = os.getcwd()+base_path +'results/'
        file_suffix = '.npy'

    pathClasslabel          = os.getcwd() + class_label_path
    pathCorners             = os.getcwd() + bbox_label_path
    pathSamples             = os.getcwd() + sample_path
    ...
    if cfgs['seq_length'] == 1:
        if cfgs['start_file'] is not None:
            file_name = cfgs['start_file']
        
        for scene_idx in range(cfgs['num_scene']): # 0 ~ 149 scenes (Total 150 scenes for nuScenes)
            for target_frame in file_list_classlabel:
                target_frame_file_name = target_frame.split('_')
                scene_info = target_frame_file_name[2]
                frame_info = target_frame_file_name[3]
                if int(scene_info) != scene_idx:
                    continue
                else:
                    print(f'Visualization for {frame_info} frame in scene number {scene_info}')
                    cfgs['lidar_path']          = f'{pathSamples}val_scene_{scene_info}_{frame_info}_ori{file_suffix}'
                    cfgs['corner_path']         = f'{pathCorners}val_scene_{scene_info}_{frame_info}_ori{file_suffix}'
                    cfgs['class_label_path']    = f'{pathClasslabel}val_scene_{scene_info}_{frame_info}_ori{file_suffix}'
                    gt_v2(cfgs = cfgs, scene_info = scene_info, frame_info=frame_info, tag = 'ori')

        if cfgs['is_augmentation']:
            lidar_path = f'{sample_path}{file_name}_aug{file_suffix}'
            corner_path = f'{sample_path}{file_name}_aug{file_suffix}'
            class_label_path = f'{class_label_path}{file_name}{file_name}_aug{file_suffix}'
            gt(cfgs = cfgs, idx = idx, lidar_path=lidar_path, class_label_path=class_label_path, bbox_label_path=corner_path, tag = 'aug')
